...
1. **下标越界**：`vector[i]` / `span[i]` 是插入运行时边界检查
   ...
   会沿用同一策略）——这是默认、受检查状态下的行为。在 `unsafe { }`
...
2. **整数溢出**：scpp 是否检查有符号溢出？**已定**：查——默认状态下
   ...
   在 `unsafe { }` 里不检查，但保证 wrap（绝不是 UB），
   靠 scpp 自己的算术 codegen 从不打 LLVM 的 `nsw`/`nuw` 做到。除以 0/
   模以 0/`INT_MIN / -1` 无条件 `abort()`，不管在不在 `unsafe { }` 里都
   一样——硬件本身就没有 wrap 后的结果可退回去用。这里故意跟 Rust 的
...
8. **可恢复错误怎么处理**：走异常，还是走值类型？**已定**：scpp 哪里都
   ...
   永久打破"把 `unsafe` 从 scpp 文件里去掉，剩下的就是能被真 C++
...
13. **去掉 `safe` 关键字：默认安全是不是比默认 opt-in 更好？** 早先的设计
    ...
    **已定：反转过来。** scpp 现在只有 `unsafe` 一个关键字——每个函数
    默认、无条件都受检查（见 [§5](ch05-static-checks.md)），不需要任何
    标注；`unsafe { }` 是一个词法作用域的块，只局部放宽
    [§5.5](ch05-static-checks.md) 那份固定的操作清单，永远不是一个整个
    函数或整个文件的逃生舱（见 [ch01](ch01-safety-context.md)）。
    "native 函数"这个概念**彻底退休**了，不只是改了个名字：现在再也
    没有办法写一个零 move/借用/生命周期检查的函数——如果一个函数的
    函数体确实需要大范围用到 [§5.5](ch05-static-checks.md) 的操作，
    表达方式是把整个函数体包进一个 `unsafe { }` 块，这样
    [§5.1](ch05-static-checks.md)-[§5.4](ch05-static-checks.md) 依然
    全程生效（见 [§1.2](ch01-safety-context.md)）——这跟 Rust 自己的
