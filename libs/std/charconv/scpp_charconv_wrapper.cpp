...
const char* scpp_std_from_chars_int64(const char* first, const char* last, std::int64_t* out_value, int* out_ec, int base) {
    std::from_chars_result result = std::from_chars(first, last, *out_value, base);
    *out_ec = static_cast<int>(result.ec);
    return result.ptr;
}

const char* scpp_std_from_chars_double(const char* first, const char* last, double* out_value, int* out_ec) {
    std::from_chars_result result = std::from_chars(first, last, *out_value);
    *out_ec = static_cast<int>(result.ec);
    return result.ptr;
}
