...
- **可失败的构造与析构**（设计已定稿，走在 `class` 完整支持之前）：
...
- **访问控制**（设计已定稿，走在 `class` 完整支持之前）：成员**变量**
...
- **内部可变性，第一阶段**（设计已定稿；回答了
  ...
  **严格得多**（真实 C++ 里 `mutable` 字段完全不设防）。一个 `mutable`
  ...
  - **永远不能**被引用或者取地址（`T&`/`const T&` 绑定、`&expr`）——
    两种都是编译错误，无条件成立，`safe`/`unsafe` 都一样。
...
[← 上一章：语法糖](ch03-syntactic-sugar.md) · [目录](README.md) · [下一章：safe 区的静态检查 →](ch05-static-checks.md)
