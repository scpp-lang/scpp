...
- **简写形式的参数包（`Concept auto&... args`）支持，但只能通过 fold
  ...
  ```
  **函数体内不支持递归拆包**（剥出第一个元素、递归处理剩下的，经典
...
- **泛型函数只用 C++20 的简写形式来写**（`void f(Concept auto& x)`）——
  完整的 `template<Concept T> void f(T& x)` 这种写法，对**函数**来说
  v0.1 **不支持**（泛型**类型**因为没有别的办法，必须用完整的
  header 形式——见 [§5.14](#514-泛型类型)）。这是刻意收窄的范围，不是
  说这两种写法在真实 C++ 里语义不一样（它们确实一样）：这样可以不用
  为**函数**专门去设计多参数模板头、显式/偏特化、模板模板参数——这个
  特性本来的目标（不靠继承实现编译期多态）用不上这些。一个连带结果：
  每个受约束的类型参数都必须绑在至少一个函数参数的声明位置上——这套
  子集里没法写"只在返回类型里出现"的泛型函数。
...
- **这一轮明确不做的**：函数体内的递归拆包（跟上面的 fold expression
  相对）、模板模板参数、默认模板实参、关联类型，以及动态分发/类型
  擦除（scpp 对应虚函数/`dyn` 的那部分，跟继承一起搁置）。泛型
  `struct`/`class` 类型，之前整个搁置，现在已经设计出来了——见
  [§5.14](#514-泛型类型)。
