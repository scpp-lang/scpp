...
- **不使用 `'a` 式生命周期语法**。不把生命周期做成独立的语法范畴（Rust 的
  ...
  **形参**分组。（设计已定稿；**尚未实现**——见 ch09 M7+。）
  ...
  - 若有 `this`/`self` 且返回引用又没有其他消歧依据，输出借用 `this`
    ...
    [§5.9](#59-方法与-this设计已定稿尚未实现)（设计已定稿，尚未实现）。
...
- **通过 `this` 把某个字段 move 出去**（比如一个 `unique_ptr` 字段）
  ...
  还没设计对应的写法；这里先标记成一个具体的后续工作，等这一节真正实现
  的时候再定，不是这一节顺带解决的。

## 5.10 函数重载（设计已定稿，尚未实现）
...
- **按值 vs 按引用是另一条、正交的轴。** `f(T)`/`f(T&)`/`f(const T&)`
  ...
  [§5.9](#59-方法与-this设计已定稿尚未实现) 里标记过的那个坑）。
...
- **`safe` 不算进一个重载的签名里。** 两个只在 `safe` 与否上不一样的
  声明（比如 `safe void f(int);` 和 `void f(int);`）不算两个重载——
  它们是同一个签名声明了两遍，是普通的重复定义错误。`safe` 是叠加在
  真实 C++ 签名之上的 scpp 专属标注，不属于重载决议要分派依据的那个
  "erasure 可见"的类型身份。
