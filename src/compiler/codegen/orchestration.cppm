    ...
    [[nodiscard]] std::expected<llvm::LLVMModuleRef, CodegenError> Codegen::generate(const Program& program)
{
        ...
        for (const GlobalVar& global : program.globals) {
            ...
            [[scpp::unsafe]] {
                ...
                variable = llvm::LLVMAddGlobal(module_, llvm_type, mangle_global_symbol_name(global.decl->var_name).c_str());
                ...
                if (global.decl->resolved_alignment != 0) llvm::LLVMSetAlignment(variable, static_cast<unsigned int>(global.decl->resolved_alignment));
            }
            globals_.emplace(global.decl->var_name, Codegen::GlobalSlot{variable, global.decl->type,
        ...
        for (const Function& fn : program.functions) {
            ...
            if (!fn.owning_module.empty() && fn.owning_module != program.module_name && fn.is_exported) {
    ...
    [[nodiscard]] std::expected<void, CodegenError> Codegen::build_overload_names()
{
        ...
        if (program_ != nullptr) {
            ...
            [[scpp::unsafe]] {
                for (const Function& fn : prog->functions) {
                    if (by_name.find(fn.name) == by_name.end()) {
                        names.push_back(fn.name);
                    }
                    const Function* fn_ptr = &fn;
                    by_name[fn.name].push_back(std::move(fn_ptr));
                }
            }
        }
    ...
    [[nodiscard]] std::expected<void, CodegenError> Codegen::define_global_initializers(const Program& program)
{
        ...
        for (const GlobalVar& global : program.globals) {
            ...
            if (global.decl->has_ctor_args) {
                ...
                std::optional<unsigned int> target_align = global.decl->resolved_alignment != 0
                    ? std::optional<unsigned int>{static_cast<unsigned int>(global.decl->resolved_alignment)}
                    : alignment_for_type(global.decl->type);
                Codegen::LValue target{it->second.global, global.decl->type, std::move(target_align)};
            ...
            std::optional<unsigned int> store_align = global.decl->resolved_alignment != 0
                ? std::optional<unsigned int>{static_cast<unsigned int>(global.decl->resolved_alignment)}
                : alignment_for_type(global.decl->type);
