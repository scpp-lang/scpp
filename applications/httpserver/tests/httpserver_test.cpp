...
#ifndef SCPP_HTTPSERVER_BINARY_PATH
#error "SCPP_HTTPSERVER_BINARY_PATH must be defined"
#endif
...
pid_t start_server(const std::filesystem::path& root, int port) {
    pid_t pid = fork();
    if (pid != 0) return pid;
    setenv("SCPP_HTTPSERVER_ROOT", root.c_str(), 1);
    std::string port_text = std::to_string(port);
    setenv("SCPP_HTTPSERVER_PORT", port_text.c_str(), 1);
    execl(SCPP_HTTPSERVER_BINARY_PATH, SCPP_HTTPSERVER_BINARY_PATH, nullptr);
    _exit(127);
}
...
void run_integration_test() {
    ...
    pid_t pid = start_server(root, port);
