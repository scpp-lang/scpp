...
| `44_switch_and_operators` | `switch`/`case`, explicit `[[fallthrough]]`, namespaced enum `case` labels, prefix/postfix `++`/`--`, compound assignment operators, and member `operator==`/`operator!=` |
...
| `46_recent_stdlib_types` | `std::shared_ptr`/`std::unique_ptr` `nullptr` comparisons, `std::vector`, `std::optional`, `std::unordered_set` with custom `hash<T>`/`equal_to<T>`, and `std::string`/`std::string_view` literal comparison and `size_t`-typed size APIs |
| `47_recent_regressions` | forwarding-reference collapse, fixed rvalue-reference rejection, lambda reference-capture constness, projected mutable-reference writes, and null returns from lifetime-tied pointer functions |
