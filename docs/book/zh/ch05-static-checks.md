...
- **不使用 `'a` 式生命周期语法**。不把生命周期做成独立的语法范畴（Rust 的
  ...
  **形参**分组。
  - **当前编译器状态。** 本节描述的多组跨函数生命周期机制，是已经定下来的
    目标设计，但当前编译器还没有实现。今天真正可用的，仍只有旧的"零标注"
    子集：恰好一个合格的引用形参，再加上 [§5.9](#59-方法与-this) 里的
    `this`-elision 特例。形参侧的 `[[scpp::lifetime(name)]]` 目前只是为
    前向兼容而被解析后忽略；下面描述的"返回组"写法也还没有真正生效。
...
**当前编译器状态。** 下面这套可恢复错误设计已经定稿，以
`std::expected<T, E>` 为中心；但当前编译器/stdlib 还没有提供这个类型。
今天只要在代码里写出 `std::expected`，依然会被直接拒绝。下面整节记录的是
它落地之后应当遵循的目标设计。
