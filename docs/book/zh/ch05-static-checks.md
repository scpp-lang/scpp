...
- **悬垂检查**：函数体内每条 `return` 语句，若返回类型是引用，编译器
  会沿借用链把返回表达式（裸变量、`a.b`、`arr[i]`，或对另一个"返回
  引用"函数的调用，递归展开）解析回其根，并要求这个根**恰好就是**上面
  省略规则选中的那个引用参数——否则报错拒绝。这就是"该函数返回的引用
  是否会悬垂"这个问题在 v0.1 里的具体判定方法。
- v0.1 已知限制（留给后续版本）：
  - 引用不能指向 `std::unique_ptr`（语言里还没有解引用/箭头语法可以
    命名"`unique_ptr` 指向的那个地方"，这是独立的前置语法工作）。
  - 调用一个返回引用的函数时，其结果只能当作**普通值**使用（自动解
    引用读取，例如 `int y = get_ref(x);`）；不能把它绑定给一个新的
    具名引用变量（`int& r = get_ref(x);`），也不能把它继续当作引用
    实参传给另一个函数。
