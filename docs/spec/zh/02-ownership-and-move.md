...
*attribute-specifier-seq*（[dcl.attr.grammar]）里，并且附着于下列位置内部的
一个**可用出现处（eligible occurrence）**：

  (13.1) 一个参数声明内部（其中也包括 `requires(...)` 表达式里的 probe
  parameter 声明）；或者

  (13.2) 一个函数或者成员函数的声明返回类型内部。

就本小节而言，一个可用出现处是：

  (13.3) 一个引用类型、指针类型、`std::span<T>` 或者
  `std::span<const T>`；或者

  (13.4) 一个 `std::reference_wrapper<T>` 里的被包装 type-id `T`；仅对本
  小节而言，这个 `T` 表示该特化里携带的那个引用。

一个 `[[scpp::lifetime(name)]]` attribute，会附着到同一个声明里、在它左边
离它最近、并且在语法上先于它出现的那个可用出现处上。如果不存在这样的可用
出现处，程序就是不合法（ill-formed）的。

【注：这也包括已经支持的那种特例：离该 attribute 最近的可用
出现处，恰好就是参数自己的整个声明类型，或者函数自己的整个声明返回类型。
它也包括写在更大类型内部的嵌套出现处，例如
...
`[[scpp::lifetime(any)]]` 的、位于参数声明内部的可用出现处，都表示一个全新的、
由编译器合成的生命周期分组；它跟下列每一项都不同：

  (16.1) 任何用户写出的分组；并且

  (16.2) 任何别的 `any` 出现处，包括同一个声明里的另一个这种可用出现处。

一个 `any` 分组不会引入一个之后还可以被返回注解或者别的出现处再次引用的
名字。

(17) 如果一个参数声明里的某个可用出现处带有
`[[scpp::lifetime(name)]]`，并且 `name` 是用户写出的分组名，那么这个出现处
就是分组 `name` 的成员。一个参数声明里的可用出现处，如果没有
`scpp::lifetime` attribute，就不属于任何具名生命周期分组。

(18) 如果一个这类函数或者成员函数的声明返回类型里的某个可用出现处带有
`[[scpp::lifetime(name)]]`，那么由这个出现处所表示的、被返回出来的那个引用、
指针或者 span，就绑定到分组 `name`。如果出现下列任一情况，程序不合法
（ill-formed）：
  ...
  (18.2) 下列两者都不满足：
    ...
    (18.2.1) 这个声明里的某个显式参数出现处属于分组 `name`；或者

    (18.2.2) 这个声明是一个 non-static 成员函数 `operator->`，并且使用了
    (23) 里的那个“隐式对象 special rule”。

(19) 一个绑定到分组 `name` 的、被返回出来的引用、指针或者 span，只能从下列来源导出：

  (19.1) 一个或多个属于分组 `name` 的显式参数出现处；或者
...
不合法（ill-formed）：
  ...
  (19.4) 另一个不同具名分组里的显式参数出现处；

  (19.5) 一个带 `any` 标签的显式参数出现处；或者

  (19.6) 一个局部对象、临时对象，或者其它没有被证明能活过这次调用的状态。

(20) 如果多个显式参数出现处属于同一个具名分组，那么这个函数可以在任何
“需要该分组”的地方，返回或者转发一个从这些出现处中的任意一个导出的值。
在调用点，一个绑定到该分组的结果，会被视为至多和该分组里所有出现处所对应的
那些实际实参组成部分中“最短寿命的那个”一样长。不同具名分组里的出现处，在
生命周期上彼此独立，除非本文档的其它规则又把它们关联起来。
...
那个可用返回出现处。至于一个从该分组导出的值，能不能被嵌进对象状态里，则单独由
(24) 约束。`requires(...)` 表达式拿带 `[[scpp::lifetime(name)]]` 的 probe
parameter 去测试某个 callable 时，也使用同样这种“声明局部、按 α-等价比较”的
规则。这样的注解会影响 concept 是否满足；它不只是给一个原本普通的测试调用加了个
语法糖。对这种满足性检查来说：

  (22.1) 如果一个 probe parameter 里的某个出现处带的是用户写出的分组名，
  那么被选中的 callable 声明里，对应的那个出现处必须属于某个非 `any`
  分组；而且如果多个 probe 出现处属于同一个用户写出的分组，那么那个声明里
  对应的多个出现处也必须同属一个分组；

  (22.2) 如果 probe 出现处属于不同的用户写出分组，那么那个声明里对应的
  出现处也必须属于不同分组；

  (22.3) 如果一个 probe parameter 里的某个出现处带的是
  `[[scpp::lifetime(any)]]`，那么被选中的 callable 声明里，对应出现处也必须
  带 `[[scpp::lifetime(any)]]`；并且

  (22.4) 如果 probe parameter 里的一个可用出现处没有
  `scpp::lifetime` attribute，那么除了 probe 自身普通的 well-formedness /
  type requirements 之外，它不会再额外施加生命周期分组约束。

(23) 一个非 static 成员函数，可以像自由函数一样，在它的显式参数声明内部的
可用出现处上使用具名生命周期分组。就本小节而言，一次对非 static 成员函数的
调用，会额外提供一个引用类型的隐式对象参数：对非 `const` 成员函数来说是 `C&`，
对 `const` 成员函数来说是 `const C&`；任何经由这个隐式对象参数形成的 borrow
或 reborrow，都受 6.2(7)-(12) 约束。这个隐式对象参数本身不能在声明里带
`[[scpp::lifetime(name)]]`。除了下面为 `operator->` 特别规定的情况之外，它也
不会单独引入一个用户写出的分组名。因此，如果一个成员函数显式写了
`[[scpp::lifetime(name)]]` 返回注解，那么除非它的某个显式参数出现处属于分组
`name`，否则这个程序就是不合法（ill-formed）的；一个只从 `this` 导出的值，
不能满足这个要求。不过，一个名为 `operator->` 的 non-static 成员函数，可以在
它的声明返回类型内部使用 `[[scpp::lifetime(name)]]`，把返回值直接绑定到那次
调用的隐式对象参数上，而不是绑定到某个显式参数上。在这个 special case 里，
`name` 仍然是一个“声明局部”的、用户写出的分组名；但对于那一个声明而言，在
(18.2.2) 以及 (19.2)-(19.3) 的语义里，它表示的就是那个隐式对象参数。
...
【注：在 `inner_form` 里，`a` 只给 `std::reference_wrapper` 携带的那个被引用对象
命名生命周期；参数对象 `w` 本身仍然是按值传递的，而它作为局部参数对象的普通
生命期，跟分组 `a` 没有关系。相反，在 `outer_form` 里，`a` 只命名外层那个
引用参数本身；`std::reference_wrapper` 内层携带的那个被引用对象，在这个例子里
仍然没有注解。——注释结束】
...
// 不合法：普通 `int` 不是可用出现处，所以按 (13) 这个 attribute
// 不会附着到任何可用出现处上
```
