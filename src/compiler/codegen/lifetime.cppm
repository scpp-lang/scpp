...
namespace scpp {
    ...
    void* Codegen::codegen_materialize_rvalue_reference_source(const Expr& expr)
{
        ...
        void* temp = create_entry_block_alloca(scpp::llvm::Value::type_of_handle(value), "rvaluetmp");
        scpp::llvm::Builder::store(builder_, value, temp);
        return temp;
    }


    void* Codegen::codegen_materialize_const_reference_source(const Expr& expr, const Type& target_type)
{
        ...
        void* temp = create_entry_block_alloca(llvm_type, "constreftmp");
    ...
    void Codegen::codegen_copy_construct_class(void* dest_ptr, void* src_ptr, const std::string& class_name)
{
        if (const Function* user_ctor = find_user_declared_copy_ctor_ast(class_name)) {
            void* ctor = scpp::llvm::Function::get_named_handle(module_, overload_names_.at(user_ctor).c_str());
    ...
    void* Codegen::find_destructor(const std::string& class_name)
{
        for (const Function& fn : program_->functions) {
            ...
            return scpp::llvm::Function::get_named_handle(module_, overload_names_.at(&fn).c_str());
        }
        return nullptr;
    }
    ...
    [[nodiscard]] const Function* Codegen::find_destructor_ast(const std::string& class_name) const
{
    ...
    void Codegen::emit_interface_destructor_dispatch_call(const std::string& interface_name, void* interface_value)
{
        ...
        void* dispatch_ptr = extract_interface_dispatch_ptr(interface_value);
        void* thunk_type = interface_dispatch_function_type(*destructor);
        ...
        void* slot_ptr = scpp::llvm::Builder::gep(builder_, table_type, dispatch_ptr, slot_indices, 2, "iface.dtor.slot");
        void* target_ptr = scpp::llvm::Builder::load(builder_, scpp::llvm::Type::get_pointer_handle(context_, 0), slot_ptr, "iface.dtor.target");
    ...
    [[nodiscard]] bool Codegen::is_field_copy_assignable(const Type& type)
{
    ...
    void Codegen::codegen_memberwise_copy_construct(void* dest_ptr, void* src_ptr,
                                            const std::string& class_name)
{
        ...
        for (std::size_t i = 0; i < info.field_names.size(); i++) {
            ...
            void* dest_field = scpp::llvm::Builder::struct_gep(builder_, info.llvm_type, dest_ptr, info.physical_field_index(i),
                                                          info.field_names[i].c_str());
            void* src_field = scpp::llvm::Builder::struct_gep(builder_, info.llvm_type, src_ptr, info.physical_field_index(i),
                                                         info.field_names[i].c_str());
            if (field_type.kind == TypeKind::Named && structs_.contains(field_type.name)) {
                ...
                void* llvm_field_type = to_llvm_type(field_type);
                void* value = scpp::llvm::Builder::load(builder_, llvm_field_type, src_field, "copiedfield");
    ...
    void Codegen::codegen_memberwise_copy_assign(void* dest_ptr, void* src_ptr, const std::string& class_name)
{
        const StructInfo& info = structs_.at(class_name);
        for (std::size_t i = 0; i < info.field_names.size(); i++) {
            const Type& field_type = info.field_types[i];
    ...
    [[nodiscard]] bool Codegen::class_has_destructor_in_chain(const std::string& class_name)
{
    ...
    void Codegen::emit_destructor_chain_calls(const std::string& class_name, void* object_ptr)
{
        if (void* dtor = find_destructor(class_name)) {
        ...
        if (def != nullptr) {
            ...
            for (auto it = interface_bases.rbegin(); it != interface_bases.rend(); ++it) {
                ...
                if (void* dtor = find_destructor((*it)->name)) {
                    build_call(dtor, {object_ptr});
                }
            }
        }
    ...
    void* Codegen::create_moved_flag_if_has_destructor(const std::string& class_name)
{
        if (!class_has_destructor_in_chain(class_name)) return nullptr;
        void* flag = create_entry_block_alloca(scpp::llvm::Type::get_int1_handle(context_), "movedflag");
        scpp::llvm::Builder::store(builder_, scpp::llvm::Value::const_int_handle(scpp::llvm::Type::get_int1_handle(context_), 0, /*SignExtend=*/false), flag);
        return flag;
    }


    void Codegen::codegen_call_destructor_chain_unless_moved(const std::string& class_name, void* object_ptr,
                                                    void* moved_flag)
{
        ...
        void* was_moved = scpp::llvm::Builder::load(builder_, scpp::llvm::Type::get_int1_handle(context_), moved_flag, "wasmoved");
        void* current_fn = scpp::llvm::BasicBlock::parent_handle(scpp::llvm::Builder::insert_block_handle(builder_));
        void* then_bb = scpp::llvm::BasicBlock::create_handle(context_, current_fn, "dtorcall");
        void* merge_bb = scpp::llvm::BasicBlock::create_handle(context_, current_fn, "dtorskip");
        scpp::llvm::Builder::cond_br(builder_, was_moved, merge_bb, then_bb);
        scpp::llvm::Builder::position_at_end_handle(builder_, then_bb);
        emit_destructor_chain_calls(class_name, object_ptr);
    ...
    void Codegen::codegen_destroy_old_class_state_for_move_assign(void* ptr, const std::string& class_name,
                                                         void* moved_flag)
{
        ...
        for (std::size_t i = 0; i < info.field_types.size(); i++) {
            const Type& field_type = info.field_types[i];
            if (field_type.kind == TypeKind::Named && structs_.contains(field_type.name)) {
                void* field_ptr = scpp::llvm::Builder::struct_gep(builder_, info.llvm_type, ptr, info.physical_field_index(i),
                                                             info.field_names[i].c_str());
