...
private:
    ...
    [[nodiscard]] bool looks_like_type_start() const {
        ...
        if (check_std_qualified("span") || check_scpp_qualified("storage_for")) return true;
    ...
    [[nodiscard]] bool looks_like_type_start_at(size_t offset) const {
        ...
        if (tok.kind == TokenKind::Identifier && peek_at(offset + 1).kind == TokenKind::ColonColon &&
            peek_at(offset + 2).kind == TokenKind::Identifier &&
            ((tok.text == "std" && peek_at(offset + 2).text == "span") ||
             (tok.text == "scpp" && peek_at(offset + 2).text == "storage_for"))) {
            return true;
        }
    ...
    Type parse_unqualified_type(bool const_qualifies_first_pointer = false) {
        if (check_scpp_qualified("storage_for")) {
            consume_scpp_qualified();
            expect(TokenKind::Less, "'<'");
            Type type;
            type.kind = TypeKind::Named;
            type.name = "scpp::storage_for";
            if (check(TokenKind::Greater)) {
                const Token& err_tok = peek();
                throw ParseError(err_tok.line, err_tok.column,
                                 "'scpp::storage_for' requires at least one type argument");
            }
            do {
