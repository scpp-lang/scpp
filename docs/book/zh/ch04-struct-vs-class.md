...
- **Move 构造和 move 赋值永远不是用户写的——编译器总是自己提供**：
  ...
  deprecated，不是删除——[depr.impldec]）彻底堵死。scpp 这条规则跟
  scpp 自己 `struct`/`class` 在 copy 这件事上已经定下的分界线
  （[§4.1](#41-struct纯平凡数据trivial-aggregate) 的 `struct` 永远能
  按位拷贝，v0.1 里 `class` 默认完全不能拷贝）是同一个道理：move 也
  永远只有一种，结构化、编译器自己的，不给作者自定义逻辑留出错的
  空间。这次决定的完整记录见 [§8](ch08-open-questions.md) Q14。
