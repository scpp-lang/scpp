...
- **`std::move(...)` 的局部变量覆盖** 现在同时涵盖稳定的类路径与当前实现对非类类型的拒绝：
  - 类类型局部变量已经有正向路径和 moved-from 后再次使用必须被拒绝的用例
  - primitive、enum class、pointer 局部变量现在各自都有一个
    `COMPILE_ERROR` 用例来记录当前 `main` 的实际行为
  - 如果并行进行的 `std::move` 泛型化调查最终放宽了当前限制，这三个
    非类用例届时需要回头改写
