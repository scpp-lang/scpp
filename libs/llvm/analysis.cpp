// analysis.cpp
//
// `llvm:analysis`: the `:analysis` partition of module `llvm` (see
...
// a way to reach official LLVM-C's `llvm-c/Analysis.h` surface via
// `import llvm;` (which re-exports every partition, this one included)
// instead of `#include <llvm-c/Analysis.h>` -- scpp (the language) has no
...
// core.cpp, `llvm:types`'s types.cpp, `llvm:debug_info`'s debug_info.cpp,
// `llvm:target`'s target.cpp, and `llvm:target_machine`'s
// target_machine.cpp, there is no aspiration here for this specific file
// to also be scpp-parseable today. `export module llvm:analysis;` below
// is nonetheless unrestricted, standard C++20 module-partition syntax --
// real ISO C++, nothing scpp-specific about it -- so the resulting
// compiled module interface will still be `import`able the same way from
...
// needs). This partition's own original introduction (as the standalone
// module `llvm.analysis`, before this consolidation) removed the *last*
...
// bridge rather than a temporary stepping stone like this file was, and is
// not part of `src/` itself).
//
...
// `import :types;` (see core.cpp/debug_info.cpp/target.cpp/
// target_machine.cpp). Like `llvm:debug_info`, `llvm:target`, and
// `llvm:target_machine` themselves, this partition does *not*
// `export import :types;`: its one current consumer (orchestration.cppm)
// already reaches both aliases through its own single `import llvm;`
// (which brings in every partition, `:core` -- itself re-exporting
// `:types` -- included), so re-exporting the same names a second time
// here would only widen this partition's own public surface with no
// consumer needing it through this path -- a plain `import :types;` still
// makes both aliases nameable within this file's own module purview below
// (which is all this file itself needs), and reachable to any importer of
// this module that sees the exported function signature below mentioning
// them.
//
...
// it is declared directly here, in this partition's own purview, rather
// than added to `llvm:types`. Unlike an opaque handle's struct tag, a
// plain enum declaration has none of the cross-module attachment
// restriction discussed at length in types.cpp's own header comment (that
// restriction is specific to class/struct/union tags, not enums), so it
// needs no global-module-fragment placement of its own. This partition
// introduces no new opaque handle struct tag at all -- its one function
// below only takes/returns handle kinds `llvm:types` already declares --
// so this file's own global module fragment is empty, exactly like
// `llvm:core`'s own core.cpp.
//
// This module declares no RAII wrapper of its own: its raw
// `LLVMVerifyModule` declaration *is* the public surface
