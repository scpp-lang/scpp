...
因为它本来就不是 `unsafe { }` 会放宽的东西。整数溢出检查
（[§5.8](#58-整数溢出设计已定稿尚未实现)）**确实**跟这个列表一样在
`unsafe { }` 里被放宽，但理由跟上面几条不一样：不是因为算术运算本来就
不合法（它从来都合法），而是因为跳过这一条检查完全没有"损坏的记账可能
泄漏到外面的 safe 代码"这种风险——这也是 §5.1-§5.4 那批检查必须无条件
继续跑的理由——详见 [§5.8](#58-整数溢出设计已定稿尚未实现)。
