...
namespace scpp {
    ...
    void Codegen::codegen_stmt(const Stmt& stmt, void* current_function)
{
        ...
        switch (stmt.kind) {
            case StmtKind::Block:
                ...
                for (const auto& s : stmt.statements) {
                    ...
                    if (scpp::llvm::BasicBlock::terminator_handle(scpp::llvm::Builder::insert_block_handle(builder_)) != nullptr) break;
            ...
            case StmtKind::VarDecl: {
                ...
                if (stmt.type.kind == TypeKind::Reference) {
                    ...
                    void* slot =
                        create_entry_block_alloca(scpp::llvm::Type::get_pointer_handle(context_, 0), stmt.var_name, declared_alignment);
                    scpp::llvm::Builder::store(builder_, referent_addr, slot);
                    locals_[stmt.var_name] = LocalSlot{slot, stmt.type};
                    locals_[stmt.var_name].is_const = stmt.is_const || stmt.is_constexpr;
                    maybe_emit_local_debug_decl(stmt.var_name, stmt.type, slot, stmt.loc);
                    if (!scope_stack_.empty()) {
                ...
                if (stmt.type.kind == TypeKind::Span) {
                    ...
                    void* span_type = to_llvm_type(stmt.type);
                    void* span_value = codegen_span_value_for_target(*stmt.init, stmt.type);
                    void* slot = create_entry_block_alloca(span_type, stmt.var_name, declared_alignment);
                    scpp::llvm::Builder::store(builder_, span_value, slot);
                    locals_[stmt.var_name] = LocalSlot{slot, stmt.type};
                    locals_[stmt.var_name].is_const = stmt.is_const || stmt.is_constexpr;
                    maybe_emit_local_debug_decl(stmt.var_name, stmt.type, slot, stmt.loc);
                    if (!scope_stack_.empty()) {
                ...
                void* llvm_type = to_llvm_type(stmt.type);
                void* slot = create_entry_block_alloca(llvm_type, stmt.var_name, declared_alignment);
                if (stmt.has_ctor_args) {
                    ...
                    if (ctor_def->eval_mode == FunctionEvalMode::Consteval) {
                        void* value = codegen_constructed_class_value(stmt.type.name, stmt.ctor_args, ctor_def);
                    ...
                    std::vector<void*> args = codegen_call_args(stmt.ctor_args, ctor_def, /*param_offset=*/1);
                ...
                if (stmt.init) {
                    ...
                    void* init_value = codegen_value_for_target(*stmt.init, stmt.type);
            ...
            case StmtKind::Return: {
                ...
                if (value != nullptr) {
                    scpp::llvm::Builder::ret(builder_, value);
                } else {
            ...
            case StmtKind::If: {
                ...
                void* cond = codegen_contextual_bool_i1(*stmt.condition);
                void* then_block = scpp::llvm::BasicBlock::create_handle(context_, current_function, "if.then");
                void* else_block = scpp::llvm::BasicBlock::create_handle(context_, current_function, "if.else");
                void* merge_block = scpp::llvm::BasicBlock::create_handle(context_, current_function, "if.end");
                ...
                if (scpp::llvm::Value::first_use_handle(scpp::llvm::BasicBlock::as_value_handle(merge_block)) != nullptr) {
                    return;
                }
            ...
            case StmtKind::While: {
                void* cond_block = scpp::llvm::BasicBlock::create_handle(context_, current_function, "while.cond");
                void* body_block = scpp::llvm::BasicBlock::create_handle(context_, current_function, "while.body");
                void* end_block = scpp::llvm::BasicBlock::create_handle(context_, current_function, "while.end");

                scpp::llvm::Builder::br(builder_, cond_block);

                scpp::llvm::Builder::position_at_end_handle(builder_, cond_block);
                // Same bool_to_i1 narrowing as the If case above.
                void* cond = codegen_contextual_bool_i1(*stmt.condition);
                scpp::llvm::Builder::cond_br(builder_, cond, body_block, end_block);
                ...
                scpp::llvm::Builder::position_at_end_handle(builder_, body_block);
                push_scope();
                ...
                scpp::llvm::Builder::position_at_end_handle(builder_, end_block);
                return;
            }
            ...
            case StmtKind::Break:
                if (!loop_stack_.empty()) {
                    ...
                    scpp::llvm::Builder::br(builder_, loop_stack_.back().end_block);
                }
                return;
            ...
            case StmtKind::Continue:
                if (!loop_stack_.empty()) {
                    emit_scope_cleanup_to_depth(loop_stack_.back().scope_depth);
                    scpp::llvm::Builder::br(builder_, loop_stack_.back().cond_block);
                }
                return;
        }
    }
    ...
    void Codegen::pop_scope()
{
        ...
        bool already_terminated = scpp::llvm::BasicBlock::terminator_handle(scpp::llvm::Builder::insert_block_handle(builder_)) != nullptr;
