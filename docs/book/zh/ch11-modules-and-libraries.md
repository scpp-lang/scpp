...
- **构建一个 module**（[§11.3](#113-导出面与接口实现单元的拆分)）
  ...
  里面（见
  [`.scppm` 模块接口格式](../../spec/zh/scppm-format.md)）。
...
- **namespace 别名复用真实 C++ 本来的别名语法**：`namespace cmath =
  ...
  别名，擦除之后交给真实 C++ 编译器就编不过
  （[ch00](ch00-design-philosophy.md) §6）。这是第二个、正交的机制，
  跟 `using foo::bar;`（只导入一个**名字**，不是整个 namespace）
  并存——两者可以自由搭配使用。
...
- **刻意不引入隐式/默认 namespace**。这条规则的早期草案本来想让 module
  ...
  namespace"这个概念——如果 scpp 自己发明一个，等擦除之后
...
extern "C" int printf(const char*, ...);  // C ABI，按 §2.1，调用永远需要 [[scpp::unsafe]] { }
```

跟 `extern "C"`（没有任何 scpp 编译器见过那个实现，
所以调用它**永远**需要 `[[scpp::unsafe]] { }`，见 §2.1）不一样，调用裸 `extern`
声明完全不需要 `[[scpp::unsafe]] { }`。信任模型不一样：module 作者把主接口单元
...
- 两个名字撞了的 module 被**直接**在同一个文件里 import：在**编译期**
  就能干净地发现（既有规则：换个名字——跟真实 C++ 一样，scpp 没有
  import 语句这一级的改名机制可以退而求其次）。
...
区分源码/二进制）定义在
[`.scppkg` 包格式](../../spec/zh/scppkg-format.md)
里。
...
- `scpp <file> --import name=path`——显式、无
  ...
  还是原始 `.scpp` 源码）。
- `scpp <file> -I <dir>`——图方便的搜索：解析 `import
...
- **归档签名**——还没有定；
  [`.scppkg` 格式](../../spec/zh/scppkg-format.md)设计成以后可以用
  一个尾部块的方式加上，不需要破坏版本号。`.scppm` 自己按设计不携带
  任何签名——它是语言层面的格式，不是分发格式（见
  [`.scppm` 模块接口格式 第 3 节](../../spec/zh/scppm-format.md)）。
...
[← 上一章：参考实现](ch10-reference-implementations.md) · [目录](README.md) · [下一章：IDE 集成 →](ch12-ide-integration.md)
