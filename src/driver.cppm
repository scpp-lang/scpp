...
[[nodiscard]] std::expected<void, DriverError> emit_object_file_for_program(Program& program, const std::string& object_path, bool emit_debug_info = false,
    ...
    // that, so -- exactly like the still-throwing ConstexprError/
    // CodegenError calls below, which belong to the not-yet-converted
    // batch 3/2 -- a disengaged result is re-thrown as-is rather than
    // absorbed into DriverError; unifying every error type onto
    // std::expected end-to-end is deferred to whichever batch finally
    // retires cli.cppm/project.cppm's own exceptions (batch 5) and updates
    // every catch site in lockstep.
