...
void run_cli_extension_tests() {
    ...
    {
        std::string case_name = "cli_project_mode_main_builds_named_executable";
        std::filesystem::path root = std::filesystem::current_path() / "cli project main!";
        std::filesystem::path exe_path = root / "cli_project_main_";
        cases_run++;
        std::filesystem::remove_all(root);
        std::filesystem::create_directories(root);
        write_text_file(root / "main.scpp", "int main() { return 9; }\n");
        RunResult build_result = run_command_capture("cd " + shell_quote(root.string()) + " && " +
                                                     shell_quote(SCPP_BINARY_PATH) + " 2>&1");
        expect(build_result.exit_code == 0,
               case_name + ": bare project build should succeed, got '" + build_result.stdout_text + "'");
        expect(std::filesystem::exists(exe_path), case_name + ": expected sanitized executable name");
        RunResult run_result = run_command_capture(shell_quote(exe_path.string()) + " 2>&1");
        expect(run_result.exit_code == 9,
               case_name + ": expected project executable exit code 9, got " + std::to_string(run_result.exit_code));
        std::filesystem::remove_all(root);
    }

    {
        std::string case_name = "cli_project_mode_lib_builds_named_artifacts";
        std::filesystem::path root = std::filesystem::current_path() / "cli project lib";
        std::filesystem::path interface_path = root / "cli_project_lib.scppm";
        std::filesystem::path archive_path = root / "libcli_project_lib.scppa";
        cases_run++;
        std::filesystem::remove_all(root);
        std::filesystem::create_directories(root);
        write_text_file(root / "lib.scpp",
                        "export module cli_project_lib;\n"
                        "namespace cli_project_lib { export int answer() { return 42; } }\n");
        RunResult build_result = run_command_capture("cd " + shell_quote(root.string()) + " && " +
                                                     shell_quote(SCPP_BINARY_PATH) + " 2>&1");
        expect(build_result.exit_code == 0,
               case_name + ": bare library project build should succeed, got '" + build_result.stdout_text + "'");
        expect(std::filesystem::exists(interface_path), case_name + ": expected project .scppm output");
        expect(std::filesystem::exists(archive_path), case_name + ": expected project .scppa output");
        std::filesystem::remove_all(root);
    }

    {
        std::string case_name = "cli_project_mode_builds_main_and_lib_together";
        std::filesystem::path root = std::filesystem::current_path() / "cli project both";
        std::filesystem::path interface_path = root / "cli_project_both.scppm";
        std::filesystem::path archive_path = root / "libcli_project_both.scppa";
        std::filesystem::path exe_path = root / "cli_project_both";
        cases_run++;
        std::filesystem::remove_all(root);
        std::filesystem::create_directories(root);
        write_text_file(root / "lib.scpp",
                        "export module cli_project_both;\n"
                        "namespace cli_project_both { export int answer() { return 42; } }\n");
        write_text_file(root / "main.scpp",
                        "import cli_project_both;\n"
                        "int main() { return cli_project_both::answer() - 42; }\n");
        RunResult build_result = run_command_capture("cd " + shell_quote(root.string()) + " && " +
                                                     shell_quote(SCPP_BINARY_PATH) + " 2>&1");
        expect(build_result.exit_code == 0,
               case_name + ": bare mixed project build should succeed, got '" + build_result.stdout_text + "'");
        expect(std::filesystem::exists(interface_path), case_name + ": expected library interface output");
        expect(std::filesystem::exists(archive_path), case_name + ": expected library archive output");
        expect(std::filesystem::exists(exe_path), case_name + ": expected project executable output");
        RunResult run_result = run_command_capture(shell_quote(exe_path.string()) + " 2>&1");
        expect(run_result.exit_code == 0,
               case_name + ": expected project executable exit code 0, got " + std::to_string(run_result.exit_code));
        std::filesystem::remove_all(root);
    }

    {
        std::string case_name = "cli_project_mode_reports_missing_sources";
        std::filesystem::path root = std::filesystem::current_path() / "cli_project_mode_reports_missing_sources";
        cases_run++;
        std::filesystem::remove_all(root);
        std::filesystem::create_directories(root);
        RunResult build_result = run_command_capture("cd " + shell_quote(root.string()) + " && " +
                                                     shell_quote(SCPP_BINARY_PATH) + " 2>&1");
        expect(build_result.exit_code != 0, case_name + ": expected bare project build to fail");
        expect(build_result.stdout_text.find("no main.scpp or lib.scpp found in the current directory") !=
                   std::string::npos,
               case_name + ": expected missing-project-file error, got '" + build_result.stdout_text + "'");
        std::filesystem::remove_all(root);
    }
