...
地继承了 `I` 的非接口 class `D`，也都必须是 thread-movable。如果 `D`
不是 thread-movable，那么 `D` 就是不合法的。

(3) 如果 `[[scpp::thread_shareable]]` 附着于某个接口 `I` 的声明，那么除了
按 8.1(6) 让 `I` 自己成为 thread-shareable 之外，任何完整对象类型直接或传递
地继承了 `I` 的非接口 class `D`，也都必须是 thread-shareable。如果 `D`
不是 thread-shareable，那么 `D` 就是不合法的。

(4) 如果某个非接口 class 直接或传递地继承了多个接口，而这些接口都按 (2)
或 (3) 带有要求，那么所有这些要求都按合取（conjunction）的方式同时生效。
只有在这个 class 满足每一条继承来的要求时，它才是合法的。

(5) (2)-(4) 所要求的检查，都在该非接口 class 的完整定义处执行；判断时
使用这个类型自己在 8.1、8.2 和 8.4 下被确定出来的 thread-movable 与
thread-shareable 值。
