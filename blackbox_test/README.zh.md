...
| `32_sizeof_storage_lifetime` | `sizeof(type)` / `sizeof(expr)`、`alignas` 修饰的原始缓冲存储、placement-new，以及显式析构调用语法 |
...
- **类按值传递/返回** 现在同时覆盖正向与反向场景：
  ...
  - 仅可移动类返回现在还额外覆盖了 `return T{...};`、裸局部变量
    隐式 move 返回，以及裸按值形参的 passthrough 返回
  - `std::string` 现在也有专门的 fresh-value 返回覆盖
  - 新的隐式 move 规则边界也被钉住了：成员访问、引用形参、
    不同类型的局部变量，以及全局对象仍然会按要求被拒绝
- **必需初始化语义现在也有专门的黑盒覆盖**：
  裸局部变量声明会被拒绝，合法的局部定义形式（`{}`、`{args}`、`= value`）
...
- **底层的 size/storage/lifetime 积木现在也有直接黑盒覆盖**：
  `sizeof(type)` / `sizeof(expr)`、`alignas` 修饰的原始缓冲存储、
