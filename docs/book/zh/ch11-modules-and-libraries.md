...
namespace mylib::math {
    extern safe int square(int x);          // 已导出，无函数体——见 §11.6
    export struct Point { int x; int y; };  // 已导出，本来就"有内容"（是数据）
}

safe int internal_helper(int x) { return x * 2; }  // 私有，有函数体——没导出，namespace 无所谓
```
...
- **完全没有 ADL（参数依赖查找），永远没有**。任何不带限定符的调用，
  ...
  一条**永久**决定，不是占位——现在不花任何代价（scpp 现在也还没有
  函数重载），但反过来做的话，会重新引入"多加一个不相关的 import，
  悄悄改变了一个已有调用的含义"这类 ADL 在真实 C++ 里出了名的诡异行为，
  跟 [ch00](ch00-design-philosophy.md) §2/§6 冲突。
- **namespace 别名复用真实 C++ 本来的别名语法**：`namespace cmath =
  ...
  别名，去掉 `safe`/`unsafe` 之后交给真实 C++ 编译器就编不过
...
namespace org::lotx::cmath {
    export safe double sqrt(double x);   // OK —— namespace 跟 module 名字匹配
}

safe double helper(double x) { return x; } // OK —— 没导出，namespace 无所谓
```
...
- **刻意不引入隐式/默认 namespace**。这条规则的早期草案本来想让 module
  ...
  namespace"这个概念——如果 scpp 自己发明一个，等把去掉 safe/unsafe 后
